...
import { useLocalVoiceInput } from "@/hooks/use-local-voice-input";
import type { ChatAttachment } from "@/lib/chat-message";
import { type ChatToolMode, type McpToolListResult, trpcClient } from "@/lib/trpc";
...
export function ChatComposerToolbar({
  ...
  const attachmentCapabilitiesQuery = useQuery({
    ...
    queryFn: () =>
      trpcClient.models.capabilities.query({
  ...
  const effective: ChatToolSelection = toolSelection ?? {
    ...
    toolIds: (workspaceToolsQuery.data ?? []).filter((t) => t.enabled).map((t) => t.id),
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (!Number.isInteger(index) || index < 0 || index >= CHAT_MODES.length) return;
  ...
  function isToolChecked(serverId: string, toolName: string) {
    ...
    const touchesServer = effective.mcpToolFilter.some((e) => e.startsWith(`${serverId}::`));
  ...
  function handleFilePick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      onAttachedFileChange({
        ...
        mimeType: file.type || (isImage ? "image/*" : isPdf ? "application/pdf" : "text/plain"),
  ...
  const mcpSummary = mcpCustomized ? `${effective.mcpServerIds.length} selected` : "MCP Server";
  ...
  const voice = useLocalVoiceInput(onVoiceResult);
  ...
  return (
    <div className="flex items-center gap-1.5">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-56">
                ...
                >
                  <Sparkles className="size-4" />
                  Skills &amp; Tools
                  {toolsAndSkillsCustomized && <Check className="ml-auto size-3.5" />}
                </DropdownMenuItem>
        ...
        {mode === "full" && (
          <>
            <button
              ...
              title={attachedFile ? "Remove attachment" : "Attach an image, PDF, or text file"}
            >
              {attachedFile?.kind === "image" ? (
                <Image className="size-3.5" />
              ) : attachedFile?.kind === "pdf" ? (
      ...
      <Popover open={modeOpen} onOpenChange={setModeOpen}>
        <PopoverTrigger asChild>
          <button type="button" className={cn(pillClass(toolMode !== "default"), "shrink-0")}>
            {(() => {
        ...
        <PopoverContent align="start" className="w-80">
          <p className="px-1 pb-2 text-xs font-medium text-muted-foreground">Mode</p>
            ...
            {CHAT_MODES.map((modeValue) => {
              ...
              return (
                ...
                >
                  ...
                  {selected && <Check className="mt-1.5 size-3.5 shrink-0 text-primary" />}
                </button>
              );
            })}
          </div>
          <p className="mt-3 border-t px-1 pt-3 text-xs text-muted-foreground">
            ...
            <strong className="text-foreground">Workspace settings → Approvals</strong>.
          </p>
        </PopoverContent>
      </Popover>
      ...
      <Popover open={toolsPickerOpen} onOpenChange={setToolsPickerOpen}>
        <PopoverTrigger asChild>
          <button type="button" className={cn(pillClass(toolsAndSkillsCustomized), "shrink-0")}>
        ...
        <PopoverContent align="start" className="w-80">
          ...
          <p className="-mt-2 mb-2 text-xs text-muted-foreground">
            By default this chat can use every skill and enabled tool in the workspace. Narrow it
            down here if this conversation should only reach a subset.
          </p>
      ...
      <div className="flex shrink-0 items-center gap-1">
        {mode === "compact" && showContextWindow && (
          <Popover open={contextOpen} onOpenChange={setContextOpen}>
            ...
            <PopoverContent align="end" className="w-64 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Context size unknown for this model.</p>
        ...
        >
          ...
          ) : voice.status === "loading" || voice.status === "transcribing" ? (
        ...
        {voice.status === "loading" && (
          <span className="text-[10px] tabular-nums text-muted-foreground">{voice.progress}%</span>
        )}
      </div>
    </div>
  );
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                size="icon"
                className="size-6"
                onClick={() => download(artifact.id, artifact.language, artifact.code)}
                aria-label="Download"
              >
