...
export default function ProjectPage() {
	...
	const [renameChatTarget, setRenameChatTarget] = useState<ChatSummary | null>(
		null,
	);
	const [renameChatTitle, setRenameChatTitle] = useState("");
	const [deleteChatTarget, setDeleteChatTarget] = useState<ChatSummary | null>(
		null,
	);
	...
	const projectsQuery = useQuery({
		...
		queryFn: () =>
			trpcClient.projects.list.query({ workspaceId: workspaceId! }),
	...
	const archiveChat = useMutation({
		mutationFn: (chatId: string) =>
			trpcClient.chats.setArchived.mutate({ chatId, archived: true }),
	...
	const duplicateChat = useMutation({
		mutationFn: (chatId: string) =>
			trpcClient.chats.duplicate.mutate({ chatId }),
		onSuccess: (chat) => {
			invalidateChats();
					...
					chats.map((chat) => (
						<ChatListItem
							...
							actions={{
								...
								onTogglePin: (c) =>
									pinChat.mutate({ chatId: c.id, pinned: !c.pinnedAt }),
				...
				<DialogContent>
					...
					<Input
						...
						onKeyDown={(e) =>
							e.key === "Enter" &&
							name.trim() &&
							renameProject.mutate(name.trim())
						}
					/>
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							This permanently deletes the chat and its messages. This action
							cannot be undone.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter showCloseButton>
						<Button
							variant="destructive"
							onClick={() =>
								deleteChatTarget && deleteChat.mutate(deleteChatTarget.id)
							}
			...
			>
				<DialogContent>
					...
					{shareTarget?.shareId && (
						<Input
							readOnly
							value={shareUrlFor(shareTarget.shareId)}
							className="text-sm"
						/>
					)}
					<DialogFooter showCloseButton>
