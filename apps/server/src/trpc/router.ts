...
import { z } from "zod";
import { cancelAgentRun, executeManagedTask, startTaskExecutionIfIdle } from "../agent-runtime";
...
import { getGoalOverview, setOrchestrationEnabled, startOrchestration } from "../goal-orchestrator";
...
async function deleteAgentGuarded(db: ReturnType<typeof getDb>, id: string) {
  ...
  const hasActiveRun = runs.some(
    (run) =>
      run.status === "pending" || run.status === "running" || run.status === "waiting_approval",
  );
  if (hasActiveRun) {
...
export const appRouter = router({
  ...
  users: router({
    ...
    get: protectedProcedure.input(z.object({ userId: z.string() })).query(({ input, ctx }) => {
  ...
  installation: router({
    status: publicProcedure.query(async () => {
      ...
      return {
        ...
        recommendedMode,
        defaultAppUrl,
        defaultWorkingDirectory: process.env.NYXEL_WORKSPACE_ROOT ?? DEFAULT_CHAT_WORKING_DIRECTORY,
        record: installation,
      };
    }),
  ...
  models: router({
    ...
    installations: workspaceProcedure
      ...
      .query(async ({ input }) => {
        const installations = await getDb().listModelInstallationsByWorkspace(input.workspaceId);
    ...
    capabilities: workspaceProcedure
      ...
      .query(async ({ input }) => {
        const providers = await getInstalledProvidersForWorkspace(input.workspaceId);
    ...
    getParameters: workspaceProcedure
      .input(z.object({ workspaceId: z.string(), modelId: z.string() }))
      .query(({ input }) => getDb().getModelParameter(input.workspaceId, input.modelId)),
    ...
    resetParameters: workspaceProcedure
      .input(z.object({ workspaceId: z.string(), modelId: z.string() }))
      .mutation(({ input }) => getDb().deleteModelParameter(input.workspaceId, input.modelId)),
    ...
    removeModelFromInstallation: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const disabledModelIds = installation.disabledModelIds.filter((id) => id !== input.modelId);
  ...
  plugins: router({
    ...
    get: protectedProcedure
      .input(z.object({ id: z.string() }))
      .query(async ({ input, ctx }) =>
        requireEntityWorkspaceOwner(ctx.user.id, () => getPlugin(input.id), "Plugin not found"),
      ),
  ...
  tools: router({
    ...
    create: workspaceProcedure
      ...
      .mutation(({ input }) => {
        ...
        const defaultSensitive = !NON_SENSITIVE_DEFAULT_TOOL_KINDS.has(input.kind);
  ...
  workspaces: router({
    list: protectedProcedure.input(z.object({ userId: z.string() })).query(({ input, ctx }) => {
      if (input.userId !== ctx.user.id) {
        throw new TRPCError({ code: "FORBIDDEN" });
      }
  ...
  chats: router({
    ...
    // chat's real id.
    getShared: publicProcedure.input(z.object({ shareId: z.string() })).query(async ({ input }) => {
      const db = getDb();
    ...
    connectWithConfig: workspaceProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (entry.transport !== "stdio" || !entry.command) {
          throw new Error(`"${entry.name}" doesn't support connecting with a config form.`);
        }
  ...
  extensions: router({
    ...
    list: workspaceProcedure
      .input(z.object({ workspaceId: z.string() }))
      .query(({ input }) => getDb().listExtensionsByWorkspace(input.workspaceId)),
    install: workspaceProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const db = getDb();
        const existing = await db.getExtensionByKey(input.workspaceId, input.key);
  ...
  seoAnalyzer: router({
    ...
    createProject: workspaceProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!ext) {
          throw new Error("The SEO/GEO/AEO Analyzer extension isn't installed in this workspace.");
        }
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (automation.triggerType === "cron" && patch.cronExpression) {
          ...
          if (!nextRunAt)
            throw new Error(`"${patch.cronExpression}" is not a valid cron expression.`);
