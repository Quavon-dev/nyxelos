...
export interface AgentRunContext {
  ...
  /**
   * Structured pause tracking (replaces text-marker detection like
   * `output.includes("pending_approval")`) — set by the caller
   * (agent-runtime.ts) that needs to know, after the run finishes, whether
   * it paused on a user question or a queued approval. Absent for callers
   * that don't care (e.g. a live chat turn already surfaces the pause via
   * the approval/question UI directly).
   */
  outcomeSignal?: RunOutcomeSignal;
  /**
   * Super-agent delegation depth/ancestor-chain (ADR-0011 + see
   * core-agent-engine's delegation-policy.ts) — 0/empty for a top-level run,
   * incremented/extended by both delegation paths (the delegate_to_agent
   * tool in delegation.ts, and agent-runtime.ts's planner-driven
   * auto-delegation branch) before recursing into a delegate's own run, so
   * neither path can recurse past MAX_DELEGATION_DEPTH or loop back to an
   * agent already in the chain.
   */
 ...
 */
function pendingApprovalResult(ctx: AgentRunContext, approvalId: string, toolLabel: string) {
  recordApprovalPause(ctx.outcomeSignal, { approvalId, toolLabel });
  return {
...
export function buildChatScopedBuiltinSkill(skillId: string, workingDirectory: string) {
  switch (skillId) {
    case "workspace_file_read":
...
// ALWAYS_REQUIRES_APPROVAL_KINDS now lives in @nyxel/core-agent-engine
// (permissions.ts) so every tool source — skills, workspace tools, workflow
// tools, delegation — reads the same central list instead of each keeping
// its own copy. Full autonomy must never mean unbounded shell execution or
// destructive file operations — these are the categories with the least
// reversibility and the largest blast radius, so "auto" mode (see
// toolPolicyForAutonomyLevel above) is not allowed to waive approval for
// them the way it does for everything else. See ADR-0017.
...
export async function buildToolsForAgent(
  ...
  const budgetTracker =
    ctx.budgetTracker ?? createAutonomyBudgetTracker(resolveAutonomyBudget(agent));

  for (const skillId of agent.skillIds) {
    const chatScopedBuiltin = ctx.workingDirectory
      ? buildChatScopedBuiltinSkill(skillId, ctx.workingDirectory)
      : null;
    const skill = chatScopedBuiltin ?? (await resolveSkillDefinition(agent.workspaceId, skillId));
    if (!skill) continue;
      ...
      execute: async (input) => {
        ...
        if (!skillKindCheck.allowed) {
          ...
          return budgetExceededResult(
            skill.id,
            skillKindCheck.reason ?? "Blocked by autonomy budget.",
          );
        }
        if (
          ...
          return pendingApprovalResult(ctx, approval.id, skill.id);
        }
        ...
        if (!skillBudgetCheck.allowed) {
          ...
          return budgetExceededResult(
            skill.id,
            skillBudgetCheck.reason ?? "Blocked by autonomy budget.",
          );
        }
    ...
    tools[workspaceToolKey] = tool({
      ...
      execute: async (input) => {
        ...
        if (
          ...
          return pendingApprovalResult(ctx, approval.id, workspaceTool.id);
        }
  ...
  for (const serverId of agent.mcpServerIds) {
    ...
    for (const mcpTool of mcpTools) {
      ...
      const toolKey =
        `${sanitizeToolNamePart(server.name)}__${sanitizeToolNamePart(mcpTool.name)}`.slice(0, 128);
      tools[toolKey] = dynamicTool({
        ...
        execute: async (input) => {
          ...
          if (
            ...
            return pendingApprovalResult(ctx, approval.id, toolKey);
          }
          ...
          if (!mcpBudgetCheck.allowed) {
            ...
            return budgetExceededResult(
              toolKey,
              mcpBudgetCheck.reason ?? "Blocked by autonomy budget.",
            );
          }
  ...
  if (ctx.taskId) {
    ...
    tools.ask_user_question = tool({
      ...
      inputSchema: z.object({
        ...
        reason: z.string().describe("Why this is urgent enough that it can't be safely assumed."),
      }),
      execute: async ({ question, reason }) => {
        ...
        recordQuestionPause(ctx.outcomeSignal, { question, reason });
        return {
  ...
  if (
    ...
    canDelegateDeeper(ctx.delegationDepth ?? 0) &&
  ...
  if (agent.autonomyLevel === "super_agent" || isAutoAssistant(agent)) {
    Object.assign(tools, await buildWorkspaceManagementTools(agent, { chatId: ctx.chatId }));
  }
