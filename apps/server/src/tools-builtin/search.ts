...
export function buildFileSearchTool(record: ToolRecord): SkillDefinition {
	...
	return {
		...
		inputSchema: z.object({
			pattern: z
				.string()
				.describe('Filename glob, e.g. "*.test.ts" — only "*" and "?" wildcards.'),
...
async function grepFiles(
	...
	for await (const file of walkFiles(roots)) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (line !== undefined && needle.test(line)) {
				results.push({ path: file, line: i + 1, text: line.trim().slice(0, 300) });
				if (results.length >= maxResults) break;
			}
			needle.lastIndex = 0;
		}
...
function buildGrepTool(
	...
	return {
		...
		async run({ query, roots, maxResults }) {
			const searchRoots = roots ?? allowedDirs;
			assertRootsAllowed(searchRoots, allowedDirs);
			const matches = await grepFiles(searchRoots, toRegExp(query), maxResults);
