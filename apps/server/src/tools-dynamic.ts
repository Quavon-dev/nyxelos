...
import { runIsolatedCustomCode } from "./plugin-sandbox";
 ...
 * kernel).
 */
export function buildDynamicToolDefinition(record: ToolRecord): SkillDefinition {
          ...
          if (!isCustomCodeSkillsEnabled()) {
            throw new Error(
              "Custom-code skills are disabled on this server. This tool kind runs " +
                "arbitrary code in an isolated subprocess (see plugin-sandbox.ts) — set " +
                "ENABLE_CUSTOM_CODE_SKILLS=true to opt in (dev only; not recommended in production).",
            );
          }
          // Runs in a dedicated, short-lived subprocess with no inherited
          // server env and a node:vm-sandboxed global scope (ADR-0007) — see
          // plugin-sandbox.ts for the full design. Replaces the previous
          // same-process `new Function(...)` call, which gave custom code the
          // server's own `process.env`/`require`/full closure access; the
          // approval workflow (sensitive: true by default) remains the
          // primary gate on what a custom-code tool is allowed to *do*.
          return runIsolatedCustomCode(code, input, {
            network: allowedHosts,
            filesystem: allowedDirs,
          });
        },
