...
**Recommendation:** See [`PLUGIN_SECURITY.md`](PLUGIN_SECURITY.md) for the full threat model and staged mitigation plan. **Update (later session):** stages 2–4 of that plan are now implemented — an install-time trust warning, best-effort ref-pinning (commit SHA resolved and stored, branch/tag installs flagged as not pinned), and a static scan (`process.env`, `child_process`, `fs.rm`/`fs.unlink`, `fetch(`, `Bun.spawn`, `eval`/`new Function`) that requires explicit user confirmation before installing anything it flags — see `apps/server/src/plugins.ts` (`scanForRiskyPatterns`, `PluginInstallNeedsConfirmationError`) and `PLUGIN_SECURITY.md`'s updated staged-plan section for exactly what each stage does and doesn't cover. **None of this is process/container isolation** — installed plugin code still runs in-process with the same access as the server itself (stage 5, still open, requires the Plugin Runtime and is explicitly out of scope here too). Backlog `BL-08`, `BL-20`.
...
| SEC-01 | Plaintext secrets at rest (DB) | P0 | **Fully fixed** — `modelInstallation.apiKey`, `knowledgeBaseConfig.obsidianApiKey`, and `mcpServer.env`/`mcpServer.oauthState` are all encrypted at rest via `encryptNullable`/`encryptJsonNullable` (`packages/db/src/crypto.ts`); the `oauthState`/`env` columns are plain `text()` (never physically `jsonb`), so no DB migration was needed. See `packages/db/src/repo/secrets-encryption.test.ts` (round-trip, null handling, and fail-closed-on-corrupted-ciphertext tests) and `packages/db/src/crypto.test.ts`. |
...
| SEC-08 | Unsigned plugin code execution | P1 | Partially mitigated — remote plugin install is disabled by default in production (`ENABLE_REMOTE_PLUGIN_INSTALL`, see `apps/server/src/feature-flags.ts`); stages 2–4 of `PLUGIN_SECURITY.md`'s mitigation plan (install-time trust warning, ref-pinning/commit-SHA recording, static scan requiring confirmation) are shipped, and stage 5 (real isolation) now covers the `custom_code` execution point specifically — see SEC-10. Signing/vetting and container-level isolation for the rest of the plugin/skill surface remain open → `BL-08`, `BL-20`. |
| SEC-09 | `terminal_run` inherits full env | P2 | Documented, approval-gated (unchanged design). `apps/server/src/approval-gate.test.ts` (added this pass) is the end-to-end Agent-Runtime-level test `BL-09` asked for — it exercises the real `buildToolsForAgent` approval-defer/reject/approve path (not just the static seed-data check in `tools-builtin-seed.test.ts`) for `terminal_run`, `custom_code`, `file_delete`, and MCP tools, across chat/task/automation/delegate ctx shapes and every autonomy level, and proves rejection never executes the underlying action while approval resumes it exactly once. `BL-09` is closed. |
| SEC-10 | `custom_code` tool runs `new Function()` in the main server process | P0 | **Fixed** — disabled by default in production (`ENABLE_CUSTOM_CODE_SKILLS`) as before, and now additionally runs in a dedicated subprocess with a caller-controlled `env` (never `process.env`) and a `node:vm`-sandboxed global scope exposing only `input`/`ctx` — `require`/`process`/`Bun` are undefined inside it. See `apps/server/src/plugin-sandbox.ts` + `plugin-sandbox.test.ts` (process-isolation and `ctx.*` permission-enforcement regression tests) and `PLUGIN_SECURITY.md`'s updated stage 5 note. This closes the specific `new Function`-in-main-process gap for `custom_code`; it does not sandbox the rest of the skill/plugin runtime (unchanged, see SEC-03). |
| SEC-11 *(new)* | Workflow HTTP Request node had no SSRF protection | P1 | **Fixed** — `apps/server/src/workflow-runner.ts`'s `http_request` node now goes through `apps/server/src/safe-fetch.ts` (scheme allowlist, blocks loopback/private RFC1918/link-local/metadata addresses on both the literal hostname and every DNS-resolved address including redirect hops, request timeout, redirect cap, response-size cap) instead of a raw `fetch`. Unattended (automation-triggered) runs additionally require `ENABLE_WORKFLOW_AUTOMATION_HTTP=true` in production — off by default, matching the `ENABLE_REMOTE_PLUGIN_INSTALL`/`ENABLE_CUSTOM_CODE_SKILLS` pattern; a manual "Run" click is never gated by this. See `safe-fetch.test.ts` and `workflow-runner.test.ts`. |
| SEC-12 *(new)* | `autonomyBudget.maxEstimatedCostUsd` was prepared but never enforced | P2 | **Fixed** — `apps/server/src/autonomy-budget.ts`'s `checkModelCallCostBudget` now runs *before* every top-level model call a run makes (planning, direct execution, super-agent synthesis — see `agent-runtime.ts`), projecting the call's worst-case cost from the model's price table and blocking outright if the projection (plus real spend recorded so far via `recordModelCallCost`) would exceed the budget, or if the model's price is unknown (fails closed on uncertainty rather than letting an unpriced model run unbounded). See `autonomy-budget.test.ts` and `agent-runtime.test.ts`'s cost-budget-preflight tests. |

Full acceptance criteria, affected files, effort, and conflict-risk-with-core-refactor notes for each `BL-*` item are in [`AGENTIC_OS_BACKLOG.md`](AGENTIC_OS_BACKLOG.md). See the "Hardening pass" section below for what changed most recently.

---

## Remediation session (earlier pass)

Fixed: SEC-00 (regression test), SEC-01 (partial — `apiKey`/`obsidianApiKey`), SEC-02, SEC-03 (docs), SEC-04, SEC-05, SEC-07, SEC-08 (partial — feature flag), plus a new SEC-10 (custom-code feature flag). Verified: SEC-06. A later pass (still before this one) closed the rest of SEC-01 (`mcpServer.env`/`oauthState` encryption) and wired `bun test` into CI after removing the 5 dead `/tests/` files.

## Hardening pass (this pass)

Fixed/closed this pass: SEC-09 (`BL-09`'s end-to-end approval-gate test), SEC-10 (subprocess + `node:vm` isolation for `custom_code`, on top of the existing feature flag), SEC-11 *(new)* — workflow HTTP node SSRF hardening, SEC-12 *(new)* — cost-budget preflight enforcement. Also fixed three latent test-hermeticity bugs found while getting `bun test` fully green from a clean checkout (see `TESTING_STRATEGY.md`): `plugins.test.ts`, `skills-resolve.test.ts`, and part of `seo-analyzer.test.ts` called `getDb()` directly and relied on an already-migrated ambient dev database rather than a throwaway per-test one — they passed on a dev machine with a pre-existing `apps/server/nyxel.sqlite` but would fail on a genuinely fresh CI checkout (`no such table: user`). Both `chat-top-bar.tsx` and `packages/create-nyxel`'s previously-noted build/typecheck failures are already fixed on this branch (verified: `bun run typecheck` and `bun run build` are green across the whole monorepo).

**Remaining risks, not fixed this pass:**
- **Plugin/skill process isolation (ADR-0007)** is still open for everything *except* `custom_code` (SEC-10): plugin-contributed `SKILL.md` bodies and hand-written skills still run in-process. The feature flags (`ENABLE_REMOTE_PLUGIN_INSTALL`, `ENABLE_CUSTOM_CODE_SKILLS`) and `PLUGIN_SECURITY.md` stages 2–4 (trust warning, ref-pinning, static scan + confirmation gate) reduce exposure and raise the bar per-install, but a plugin that ships a malicious `run()` inside a hand-written/dynamic skill (not `custom_code`) still executes same-process.
- **Plugin signing/vetting** (SEC-08's original ask) is still not implemented — the feature flag and stages 2–4 are coarser mitigations (all-or-nothing per server / warn-and-confirm per install), not cryptographic per-plugin trust.
- **The `custom_code` sandbox is process + `vm`-level, not a kernel/container boundary.** The child subprocess is still a full Bun runtime outside the `vm` context; a sophisticated `vm` escape is a known theoretical risk class this doesn't claim to close. It does close the concrete, exploitable gap that existed before (same-process `new Function` with full `process.env`/`require` access).
- **`run_workflow` (the agent-tool counterpart to a workflow's "agent" node) is not itself approval-gated** — it's assigned directly in `tools.ts`'s `buildToolsForAgent` without going through the sensitive-tool/approval machinery. An autonomous agent can trigger any of the workspace's saved workflows, including ones with an HTTP Request node, without a human in the loop (though SEC-11's SSRF hardening and the `ENABLE_WORKFLOW_AUTOMATION_HTTP` gate still apply). Not fixed this pass — flagged for a future, narrowly-scoped change.
- **`maxEstimatedCostUsd`'s preflight (SEC-12) is per top-level model call, not per token generated.** It can't interrupt a single very long generation mid-stream (the AI SDK's usage is only known once a `streamText` call fully finishes) — a run with exactly one, very large call could still exceed the budget before the *next* call's preflight catches it. This is a real, documented limitation of doing this without deeper AI SDK per-step (`onStepFinish`) integration, not an oversight.
