...
| SEC-08 | Unsigned plugin code execution | P1 | Partially mitigated — remote plugin install is disabled by default in production (`ENABLE_REMOTE_PLUGIN_INSTALL`, see `apps/server/src/feature-flags.ts`); a later session added `PLUGIN_SECURITY.md` stages 2–4 (install-time trust warning, ref-pinning/commit-SHA recording, static scan requiring confirmation). Signing/vetting and real process/container isolation (stage 5) remain unimplemented — documented (known upstream) → `BL-08`, `BL-20`. |
...
- **Plugin/skill process isolation (ADR-0007)** is unchanged — the feature flag added this session (`ENABLE_REMOTE_PLUGIN_INSTALL`) reduces exposure by defaulting off in production, and a later session's `PLUGIN_SECURITY.md` stages 2–4 (trust warning, ref-pinning, static scan + confirmation gate) raise the bar per-install, but the underlying in-process trust model (SEC-03/`PLUGIN_SECURITY.md`) is not fixed, only gated and made more visible. Real process/container isolation (stage 5) is still unimplemented.
