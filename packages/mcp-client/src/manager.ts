...
function parseHttpMcpUrl(config: McpServerConfig): URL {
	...
	if (trimmed.startsWith("/")) {
		if (trimmed.startsWith("/guides/")) {
			throw new McpInvalidConfigurationError({
				serverName: config.name,
				reason: `the configured endpoint is a documentation path (${trimmed}); use the actual MCP server URL instead, for example https://mcp.notion.com/mcp.`,
			});
		}
		throw new McpInvalidConfigurationError({
			serverName: config.name,
			reason: `the configured endpoint is relative (${trimmed}); HTTP MCP endpoints must be absolute URLs like https://mcp.notion.com/mcp.`,
		});
	}
	...
	try {
		...
		try {
			...
			throw new McpInvalidConfigurationError({
				serverName: config.name,
				reason: `"${config.url}" is not a valid endpoint URL. Use the MCP endpoint itself, for example https://example.com/mcp, not a docs path.`,
			});
		}
	}
...
function createTransport(
	...
	if (config.transport === "stdio") {
		if (!config.command) {
			throw new Error(
				`MCP server "${config.name}" is configured as stdio but has no command.`,
			);
		}
	...
	if (!config.url) {
		throw new Error(
			`MCP server "${config.name}" is configured as http but has no url.`,
		);
	}
...
export class McpClientManager {
	...
	private getOrCreateOAuthProvider(
		config: McpServerConfig,
	): InMemoryMcpOAuthProvider | undefined {
		if (config.transport !== "http" || !config.oauth?.callbackUrl)
			return undefined;
	...
	private toAuthorizationRequiredError(
		serverId: string,
		serverName: string,
	): McpAuthorizationRequiredError {
		...
		if (!authorizationUrl || !callbackUrl) {
			throw new Error(
				`MCP server "${serverName}" requires authentication, but no authorization URL was produced.`,
			);
		}
	...
	async disconnectAll(): Promise<void> {
		await Promise.all(
			[...this.servers.keys()].map((id) => this.disconnect(id)),
		);
	}
	...
	async listAllTools(): Promise<McpToolSummary[]> {
		const all = await Promise.all(
			[...this.servers.keys()].map((id) => this.listTools(id)),
		);
		return all.flat();
	}
