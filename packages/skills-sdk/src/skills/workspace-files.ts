...
function resolveWorkspacePath(rootDir: string, requestedPath: string) {
  return path.isAbsolute(requestedPath) ? requestedPath : path.join(rootDir, requestedPath);
}
...
export function createWorkspaceFileReadSkill(rootDir: string) {
  return defineSkill({
    ...
    inputSchema: z.object({
      path: z
        .string()
        .describe("Workspace-relative or absolute path to a file inside the workspace."),
    }),
  ...
  return defineSkill({
    ...
    inputSchema: z.object({
      path: z
        .string()
        .default(".")
        .describe("Workspace-relative or absolute directory path inside the workspace."),
    }),
    permissions: { network: [], filesystem: [rootDir] },
    sensitive: false,
    async run({ path: requestedPath }, ctx) {
...
export function createWorkspaceFileWriteSkill(rootDir: string) {
  return defineSkill({
    ...
    inputSchema: z.object({
      path: z
        .string()
        .describe("Workspace-relative or absolute file path inside the workspace."),
      content: z.string(),
    }),
    permissions: { network: [], filesystem: [rootDir] },
    sensitive: true,
