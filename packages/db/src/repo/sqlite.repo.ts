...
export function createSqliteRepository(filePath: string): DbRepository {
  ...
  return {
    ...
    async deleteAgent(agentId) {
      ...
      // failure.
      db.update(schema.chat).set({ agentId: null }).where(eq(schema.chat.agentId, agentId)).run();
    ...
    async getGoal(goalId) {
      const row = db.select().from(schema.goal).where(eq(schema.goal.id, goalId)).get();
    ...
    async getAgentRun(id) {
      const row = db.select().from(schema.agentRun).where(eq(schema.agentRun.id, id)).get();
    ...
    async updateMcpServerOAuthState(id, oauthState) {
      db.update(schema.mcpServer).set({ oauthState }).where(eq(schema.mcpServer.id, id)).run();
    },
    ...
    async listToolsByWorkspace(workspaceId) {
      return db.select().from(schema.tool).where(eq(schema.tool.workspaceId, workspaceId)).all();
    },

    async getTool(id) {
      const row = db.select().from(schema.tool).where(eq(schema.tool.id, id)).get();
      return row ?? null;
    },
    ...
    async getMemoryEntry(id) {
      const row = db.select().from(schema.memoryEntry).where(eq(schema.memoryEntry.id, id)).get();
      return row ?? null;
    },
    ...
    async getExtension(id) {
      const row = db.select().from(schema.extension).where(eq(schema.extension.id, id)).get();
      return row ?? null;
    },
    ...
    async getPlugin(id) {
      const row = db.select().from(schema.plugin).where(eq(schema.plugin.id, id)).get();
      return row ?? null;
    },
    ...
    async deleteLibraryFolder(id) {
      db.delete(schema.libraryFolder).where(eq(schema.libraryFolder.id, id)).run();
    },
    ...
    async getLibraryFile(id) {
      const row = db.select().from(schema.libraryFile).where(eq(schema.libraryFile.id, id)).get();
      return row ?? null;
    },
    ...
    async getWorkflowRun(id) {
      const row = db.select().from(schema.workflowRun).where(eq(schema.workflowRun.id, id)).get();
      return row ?? null;
    },
    ...
    async getSeoProject(id) {
      const row = db.select().from(schema.seoProject).where(eq(schema.seoProject.id, id)).get();
      return row ?? null;
    },
    ...
    async listSeoFindingsByRun(runId) {
      return db.select().from(schema.seoFinding).where(eq(schema.seoFinding.runId, runId)).all();
    },
    ...
    async getSeoFinding(id) {
      const row = db.select().from(schema.seoFinding).where(eq(schema.seoFinding.id, id)).get();
      return row ?? null;
    },
    ...
    async getSeoBlogPost(id) {
      const row = db.select().from(schema.seoBlogPost).where(eq(schema.seoBlogPost.id, id)).get();
      return row ?? null;
    },
