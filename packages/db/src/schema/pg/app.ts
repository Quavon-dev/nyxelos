...
export const goalMilestoneStatus = pgEnum("goal_milestone_status", ["pending", "completed"]);
...
export const seoFindingCategory = pgEnum("seo_finding_category", ["seo", "geo", "aeo"]);
export const seoFindingSeverity = pgEnum("seo_finding_severity", ["info", "warning", "critical"]);
...
export const automationTargetKind = pgEnum("automation_target_kind", ["agent", "workflow"]);
...
export const libraryItemKind = pgEnum("library_item_kind", ["image", "document", "video", "other"]);
...
export const memorySource = pgEnum("memory_source", ["user", "agent", "automation", "system"]);
...
export const workspace = pgTable("workspace", {
  ...
  defaultAutonomyLevel: agentAutonomyLevel("default_autonomy_level").notNull().default("assisted"),
...
export const plugin = pgTable("plugin", {
  ...
  manifest: jsonb("manifest").notNull().default({}).$type<Record<string, unknown>>(),
  skillSlugs: jsonb("skill_slugs").notNull().default([]).$type<string[]>(),
  agentDefs: jsonb("agent_defs").notNull().default([]).$type<PluginAgentDefinition[]>(),
