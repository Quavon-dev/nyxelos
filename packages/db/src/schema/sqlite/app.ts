...
export const modelParameter = sqliteTable(
  "model_parameter",
  {
    ...
    stopSequences: text("stop_sequences", { mode: "json" }).notNull().default([]).$type<string[]>(),
...
export const agent = sqliteTable("agent", {
  ...
  toolIds: text("tool_ids", { mode: "json" }).notNull().default([]).$type<string[]>(),
...
export const automation = sqliteTable("automation", {
  ...
  targetKind: text("target_kind").notNull().default("agent").$type<AutomationTargetKind>(),
  name: text("name").notNull(),
...
export const task = sqliteTable("task", {
  ...
  input: text("input", { mode: "json" }).notNull().default({}).$type<Record<string, unknown>>(),
  plan: text("plan", { mode: "json" }).$type<Record<string, unknown> | null>(),
  handoff: text("handoff", { mode: "json" }).$type<Record<string, unknown> | null>(),
...
export const plugin = sqliteTable("plugin", {
  ...
  skillSlugs: text("skill_slugs", { mode: "json" }).notNull().default([]).$type<string[]>(),
  ...
  riskFindings: text("risk_findings", { mode: "json" }).notNull().default([]).$type<string[]>(),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull(),
});
...
export const seoAnalysisRun = sqliteTable("seo_analysis_run", {
  ...
  workspaceId: text("workspace_id")
    .notNull()
    .references(() => workspace.id, { onDelete: "cascade" }),
  status: text("status").notNull().default("running").$type<SeoAnalysisRunStatus>(),
  score: integer("score"),
