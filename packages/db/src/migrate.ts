...
function sqliteHasColumn(
	sqlite: Database,
	tableName: string,
	columnName: string,
) {
	const columns = sqlite.query(`PRAGMA table_info(${tableName})`).all() as {
		name: string;
	}[];
...
function reconcileLegacySqliteChatToolPolicyMigration(sqlite: Database) {
	if (
		!sqliteHasTable(sqlite, "chat") ||
		!sqliteHasTable(sqlite, "__drizzle_migrations")
	)
		return;
